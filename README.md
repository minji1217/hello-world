...
| Module 4‑2 | Cross Encoder 2차 Ranking | final Top‑K |

---

## 2. Module 2 — Retrieval

Semantic retrieval + bib soft bias를 통해 **context 기반 후보 Top‑150**을 선정합니다.

### 2.1 입력

| 항목 | 설명 |
|------|------|
| `context_queries` | 인용 위치별 문맥 텍스트 N개 (각 `query_id`에 대응) |
| `paper_query` | 논문 전체를 대표하는 query 1개 (optional) |
| `embedding_db` | `{ paper_id → vector }` 형태의 임베딩 DB |
| `FAISS index` | 사전 구축된 ANN 인덱스 (IndexFlatIP, L2-normalized) |
| `bib_ids` | 유저 bibliography paper_id 목록 (optional) |

### 2.2 동작 개요

1. **Context Embedding**  
   - `context_queries` 각각을 SPECTER2에 넣어 context embedding을 생성합니다.

2. **FAISS Semantic Retrieval**  
   - 각 context embedding에 대해 FAISS 인덱스에서 cosine similarity 기준 **Top‑3000** 후보를 검색합니다.  
   - 검색 시 유사도 값을 **paper similarity**로 저장합니다.

3. **Context Similarity & Score Fusion**  
   - Top‑3000 후보에 대해 context embedding과의 cosine을 다시 계산하여 **context similarity**를 구합니다.  
   - `paper similarity`와 `context similarity`를 가중합한 통합 점수로 **Top‑1500**까지 pruning합니다.

4. **Bibliography Soft Bias & Top‑150 선정**  
   - Top‑1500 후보에 대해 user bibliography 기반 **bib_score**를 계산합니다.  
   - `final similarity = w1 * (paper+context score) + w2 * bib_score` 로 정의하여 **Top‑150** 후보를 선정합니다.
...
## 3. Module 3 — Graph Expansion & Feature Engineering

Module 2의 Top‑150 후보를 기반으로 **citation graph 확장**과 **feature 계산**을 수행합니다.
...
| `metadata_db` | `{ paper_id → { year, citation_count } }` (연도, 인용 수 등 메타데이터) |
...
1. **Graph Expansion**  
   ...
   - semantic 후보 + graph 후보를 합쳐 약 **150~300편**으로 후보 풀 확장.

2. **Feature Engineering**  
   - 확장된 모든 후보에 대해 임베딩/메타데이터/그래프 정보를 기반으로 feature를 계산하고 정규화합니다.
...
| `graph_score` | citation graph 양방향 연결 강도 (co‑citation, 공통 이웃 등 → min‑max 정규화) |
...
| `has_forward` / `has_backward` | references / cited_by 연결 존재 여부 (bool) |
| `forward_count` / `backward_count` | 참조하는 논문 수 / 인용하는 논문 수 |
| `graph_available` | 그래프 정보가 충분히 존재하는지 여부 (bool) |
| `is_target` | 학습/평가 시 정답 인용 목록에 포함 여부 (레이블) |
...
Graph Expansion까지 확장된 후보(약 225개)를 feature 기반 선형 점수로 랭킹하여 **Top‑30**으로 압축합니다.
...
| weight config | \( \alpha, \beta_1, \beta_2, \beta_3, \beta_4, \beta_5 \) (grid search로 튜닝된 가중치) |

### 4.2 Scoring 수식

각 후보 논문 \(p\)에 대해 1차 점수 \(score_1(p)\)를 다음과 같이 계산합니다.

\[
score_1(p) = \alpha \cdot semantic\_score
...
- 목적: Cross Encoder가 처리해야 할 입력 수를 줄이고, 노이즈를 제거하는 **빠른 필터링 단계**입니다.
...
> 약 225개 후보 → Top‑30 후보로 압축 (Cross Encoder 입력 수 제한)

---

## 5. Module 4‑2 — Cross Encoder (2차 Ranking)

1차 랭킹 결과 Top‑30 후보에 대해 **문맥–논문 쌍**을 입력으로 하는 Cross Encoder를 적용해, 최종 Top‑K 인용 추천을 생성합니다.
...
- **Module 2**: FAISS + bib soft bias로 context 기반 Top‑150 후보 생성  
- **Module 3**: citation graph 기반 확장 + feature engineering (semantic, bib, graph, recency 등)  
- **Module 4‑1**: 선형 결합을 이용한 1차 Ranking, Top‑30 필터링  
- **Module 4‑2**: Cross Encoder로 문맥–논문 쌍 단위 정교한 2차 Ranking → final Top‑K

이 파이프라인을 통해 대규모 논문 코퍼스에서도 **문맥 기반·개인화·그래프 정보**를 모두 반영한 인용 추천을 효율적으로 수행할 수 있습니다.
```
